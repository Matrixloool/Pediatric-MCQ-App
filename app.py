...
# ----------------------------------------------------
# 3. Defensive PEM Key Padding Runtime Fix
# ----------------------------------------------------
try:
    if is_gsheets_configured:
        if hasattr(st.secrets, "_secrets"):
            secrets_dict = st.secrets._secrets
            if "connections" in secrets_dict and "gsheets" in secrets_dict["connections"]:
                gsheets_secrets = secrets_dict["connections"]["gsheets"]
                if "private_key" in gsheets_secrets:
                    raw_key = gsheets_secrets["private_key"]
                    if isinstance(raw_key, str):
                        sanitized_key = raw_key.replace("\\n", "\n").replace("\r", "").strip()
                        gsheets_secrets["private_key"] = sanitized_key
                        logger.info("Successfully sanitized Google Sheets PEM private key padding.")
except Exception as e:
    logger.warning(f"Google Sheets secrets sanitization skipped or failed: {str(e)}")
...
# ----------------------------------------------------
# 4. Strict Deduplication Hash Utility
# ----------------------------------------------------
def clean_question_hash(question_text: str) -> str:
    """
    Normalizes a question text into a unique alphanumeric hash to guarantee strict deduplication.
    """
    return re.sub(r'[^a-z0-9]', '', question_text.lower().strip())

# ----------------------------------------------------
# 5. Persistent State Loader & Saver (Local + Cloud)
# ----------------------------------------------------
def save_progress():
    """
    ...
    state_to_save = {
        ...
        "global_question_hashes": st.session_state.get("global_question_hashes", []),
        "topic_ended": st.session_state.get("topic_ended", {})
    }
    ...
    if is_gsheets_configured:
        try:
            ...
            sheet_data = {
                ...
                "api_key": [st.session_state.get("api_key", "")],
                "global_question_hashes": [json.dumps(st.session_state.get("global_question_hashes", []))],
                "topic_ended": [json.dumps(st.session_state.get("topic_ended", {}))],
...
def load_progress() -> bool:
    ...
    if is_gsheets_configured:
        try:
            ...
            if df is not None and not df.empty:
                ...
                # Load expanded cloud sync variables
                st.session_state["global_question_hashes"] = json.loads(row.get("global_question_hashes", "[]"))
                st.session_state["topic_ended"] = json.loads(row.get("topic_ended", "{}"))
    ...
    if os.path.exists(PROGRESS_FILE):
        try:
            ...
            st.session_state["global_question_hashes"] = data.get("global_question_hashes", [])
            st.session_state["topic_ended"] = data.get("topic_ended", {})
...
def reset_all_progress():
    ...
    if is_gsheets_configured:
        try:
            ...
            empty_df = pd.DataFrame(columns=[
                ...
                "mcqs", "user_answers", "num_mcqs_per_topic", "api_key",
                "global_question_hashes", "topic_ended", "last_updated"
            ])
            conn = st.connection("gsheets", type=GSheetsConnection)
    ...
    keys_to_clear = [
        ...
        "user_answers", "current_q_idx", "global_question_hashes", "topic_ended"
    ]
...
if "filenames" not in st.session_state:
    if not load_progress():
        ...
        st.session_state["global_question_hashes"] = []
        st.session_state["topic_ended"] = {}

# Fallback initializations for cloud sync structure updates
if "global_question_hashes" not in st.session_state:
    st.session_state["global_question_hashes"] = []
if "topic_ended" not in st.session_state:
    st.session_state["topic_ended"] = {}
...
# ----------------------------------------------------
# 7. Sidebar Elements & Configuration
# ----------------------------------------------------
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Gemini Model",
        options=["gemini-3.5-flash", "gemini-1.5-flash", "gemini-1.5-pro"],
        index=0,
        help="gemini-3.5-flash is highly recommended for speed and accuracy upgrades."
    )
...
# B. ACTIVE STUDY SPACE
    ...
    if current_topic_idx >= len(topics):
        ...
        if active_topic not in st.session_state["mcqs"] or not st.session_state["mcqs"][active_topic]:
            ...
            # Count selector & "Extract All" layout
            ...
            with col_toggle:
                ...
                st.write("")
                # ----------------------------------------------------
                # 1. 'Extract All' Checkbox Option
                # ----------------------------------------------------
                extract_all = st.checkbox(
                    "Extract All MCQs verbatim",
                    value=False,
                    help="Attempts to extract every single matching MCQ available in this sub-topic textbook text at once."
                )
            
            is_pdf_available = bool(st.session_state["outline_text"])
            if not is_pdf_available:
                st.warning("⚠️ **PDF Text Context Missing**: You are studying cloud synced questions. To extract new questions, please upload your chapter PDF in the sidebar first.")
                
            if st.button("Extract Verbatim MCQs 🔍", type="primary", use_container_width=True, disabled=not is_pdf_available):
                # If Extract All is checked, we target a high count (e.g. 80) to capture everything
                target_count = 80 if extract_all else batch_count
                spinner_msg = "Gemini is parsing and extracting ALL matching verbatim MCQs..." if extract_all else f"Gemini is parsing and extracting {batch_count} verbatim MCQs..."
                ...
                with st.spinner(spinner_msg):
                    try:
                        ...
                        # ----------------------------------------------------
                        # 3. Strict Deduplication exclusions gathering
                        # ----------------------------------------------------
                        # Gather all currently loaded questions across ALL topics currently studied
                        global_existing_qs = []
                        for t_mcqs in st.session_state["mcqs"].values():
                            for q in t_mcqs:
                                global_existing_qs.append(q["question"])
                                
                        chapter_context = st.session_state["outline_text"]
                        raw_mcqs = client.extract_verbatim_mcqs(
                            subtopic=active_topic,
                            chapter_context=chapter_context,
                            num_questions=target_count,
                            existing_questions=global_existing_qs
                        )
                        
                        # Filter strictly inside the application code by Unique Hash to avoid repeats 100%
                        unique_new_mcqs = []
                        for q in raw_mcqs:
                            q_hash = clean_question_hash(q["question"])
                            if q_hash not in st.session_state["global_question_hashes"]:
                                st.session_state["global_question_hashes"].append(q_hash)
                                unique_new_mcqs.append(q)
                            else:
                                logger.info(f"Deduplicated question: {q['question'][:40]}...")
                                
                        if unique_new_mcqs:
                            st.session_state["mcqs"][active_topic] = unique_new_mcqs
                            st.session_state["current_q_idx"] = 0
                            save_progress()
                            st.success(f"Extracted {len(unique_new_mcqs)} unique questions successfully!")
                            st.rerun()
                        else:
                            st.session_state["topic_ended"][active_topic] = True
                            save_progress()
                            st.warning("No new verbatim questions found matching this subtopic in textbook.")
                            st.rerun()
                    except Exception as e:
            ...
            # ----------------------------------------------------
            # 1. & 3. Dynamic "Load More", deduplication & completion panel
            # ----------------------------------------------------
            if checked and q_idx == len(mcqs_list) - 1:
                ...
                with col_load:
                    # ----------------------------------------------------
                    # 1. Completion Message if topic is ended
                    # ----------------------------------------------------
                    if st.session_state["topic_ended"].get(active_topic, False):
                        st.markdown("""
                        <div style="background-color: rgba(239, 68, 68, 0.1); border: 1px solid #EF4444; border-radius: 10px; padding: 20px; text-align: center;">
                            <h4 style="color: #EF4444; margin-top: 0; margin-bottom: 8px; font-weight: 700;">🏁 End of Topic</h4>
                            <p style="color: #E2E8F0; font-size: 0.9rem; margin-bottom: 0;">
                                <strong>The MCQs for this topic have ended.</strong><br>
                                All available questions inside the textbook for this sub-topic have been extracted and studied.
                            </p>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        ...
                        if st.button("Load More MCQs 📥", type="primary", use_container_width=True, disabled=not is_pdf_available):
                            # Collect global questions for strict duplicate prevention in API
                            global_existing_qs = []
                            for t_mcqs in st.session_state["mcqs"].values():
                                for q in t_mcqs:
                                    global_existing_qs.append(q["question"])
                            ...
                            with st.spinner("Extracting additional verbatim questions from text..."):
                                try:
                                    ...
                                    raw_new_mcqs = client.extract_verbatim_mcqs(
                                        subtopic=active_topic,
                                        chapter_context=st.session_state["outline_text"],
                                    ...
                                    # Strict global hash deduplication
                                    unique_new_mcqs = []
                                    for q in raw_new_mcqs:
                                        q_hash = clean_question_hash(q["question"])
                                        if q_hash not in st.session_state["global_question_hashes"]:
                                            st.session_state["global_question_hashes"].append(q_hash)
                                            unique_new_mcqs.append(q)
                                    ...
                                    if unique_new_mcqs:
                                        st.session_state["mcqs"][active_topic].extend(unique_new_mcqs)
                                        st.session_state["current_q_idx"] = len(mcqs_list) # Point to the first newly added question
                                        save_progress()
                                        st.success(f"Successfully loaded {len(unique_new_mcqs)} additional MCQs!")
                                        st.rerun()
                                    else:
                                        st.session_state["topic_ended"][active_topic] = True
                                        save_progress()
                                        st.warning("No new unique MCQs found in the textbook text for this sub-topic.")
                                        st.rerun()
                                except Exception as e:
